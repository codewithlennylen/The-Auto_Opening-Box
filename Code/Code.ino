...
int openDelay = 2000; // time in milliseconds that the box will remain open (2 Seconds)
long duration;
int distance;
...
void loop() {
  ...
  // Check if there's an object close to the ultrasonic sensor then open the Box
  if (triggered == 1)
  {
    open_box();
  }
